...
class UserController extends AdminBaseController
{
    ...
    public function addPost()
    {
        if ($this->request->isPost()) {
            //$_POST['avatar']=$_SERVER['HTTP_HOST'].'/upload/'.$_POST['avatar'];
            //dump($_POST);DIE;
            if(!empty($_POST['best'])){
                foreach ($_POST['best'] as $k => $v) {
                    $new=explode('+', $v);
                    $id[]=$new[0];
                    $info[]=$new[1];
                }
                $_POST['best']=(implode(",",$id));
                $_POST['best_info']=(implode(",",$info));
            }
